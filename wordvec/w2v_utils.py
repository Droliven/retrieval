...
def save_dataset_doc_vector(w2v, city_root_path, len_vec=400, tfidf_weighted=False,
    ...
    city_name = city_root_path.split('\\')[-1]

    # 创建好文件夹
    fv_city_root = osp.join(fv_base_root, city_name)
    os.makedirs(fv_city_root, exist_ok=True)

    city_txt_files = glob(osp.join(city_root_path, "*.txt"))
    for txt_file in tqdm(city_txt_files):
        ...
        with open(txt_file, 'r', encoding='utf-8') as file:
            ...
            np.save(osp.join(fv_city_root, file_id), embedding)


if __name__ == '__main__':
    wv2 = load_pretrained_word2vec("./ckpt1M/word2vec_model_instaCities1M.model")
    # txt_root = "/home/liuzhian/hdd4T/datasets/instaCities1M/captions_resized_1M/cities_instagram"
    txt_root = r"F:\model_report_data\multimodal_image_retrieval\InstaCities1M\captions_resized_1M\cities_instagram"

    for city in ['chicago', 'london', 'losangeles', 'newyork', 'sanfrancisco', 'sydney', 'singapore', 'melbourne',
                 'miami', 'toronto']:
        save_dataset_doc_vector(wv2, city_root_path=osp.join(txt_root, city),
                                fv_base_root=r"F:\model_report_data\multimodal_image_retrieval\InstaCities1M\w2v_feat_vectors\cities_instagram")
